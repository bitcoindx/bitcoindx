...
This guide describes how to build bitcoindxd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build BitcoinDX Core from source.
...
### 4. Clone BitcoinDX repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the BitcoinDX Core repository to a directory.
...
It is not necessary to build wallet functionality to run `bitcoindxd` or  `bitcoindx-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/bitcoindx/bitcoindx/blob/master/doc/descriptors.md), `sqlite` is required.
...
BitcoinDX Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/bitcoindx/bitcoindx/issues/7714).
...
You can deploy a `.dmg` containing the BitcoinDX Core application using `make deploy`.
...
## Building BitcoinDX Core

### 1. Configuration

There are many ways to configure BitcoinDX Core, here are a few common examples:
...
Run the following in your terminal to compile BitcoinDX Core:
...
## Running BitcoinDX Core

BitcoinDX Core should now be available at `./src/bitcoindxd`.
If you compiled support for the GUI, it should be available at `./src/qt/bitcoindx-qt`.

The first time you run `bitcoindxd` or `bitcoindx-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/BitcoinDX/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/BitcoinDX"

touch "/Users/${USER}/Library/Application Support/BitcoinDX/bitcoindx.conf"

chmod 600 "/Users/${USER}/Library/Application Support/BitcoinDX/bitcoindx.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/BitcoinDX/debug.log
```

## Other commands:

```shell
./src/bitcoindxd -daemon      # Starts the bitcoindx daemon.
./src/bitcoindx-cli --help    # Outputs a list of command-line options.
./src/bitcoindx-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/bitcoindx-qt -server # Starts the bitcoindx-qt server mode, allows bitcoindx-cli control
```
