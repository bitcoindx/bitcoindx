# Bootstrappable BitcoinDX Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinDX
Core builds.
...
To build BitcoinDX Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinDX Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoindx-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoindx-core/guix.sigs).
