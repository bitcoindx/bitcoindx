...
// For details see https://github.com/bitcoindx/bitcoindx/pull/22348.
...
const char * const BITCOINDX_CONF_FILENAME = "bitcoindx.conf";
const char * const BITCOINDX_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitcoindx/bitcoindx/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoindx-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITCOINDX_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitcoindx";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinDX
    // macOS: ~/Library/Application Support/BitcoinDX
    // Unix-like: ~/.bitcoindx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinDX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinDX";
#else
    // Unix-like
    return pathRet / ".bitcoindx";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINDX_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitcoinDX Core copyright is not removed by accident
    if (copyright_devs.find("BitcoinDX Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitcoinDX Core developers";
    }
