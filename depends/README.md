...
**BitcoinDX Core's `configure` script by default will ignore the depends output.** In
...
RISC-V known issue: gcc-7.3.0 and gcc-7.3.1 result in a broken `test_bitcoindx` executable (see https://github.com/bitcoindx/bitcoindx/pull/13543),
...
options will be passed to bitcoindx's configure. In this case, `--disable-wallet`.
