...
The BitcoinDX Core daemon can be configured to act as a trusted "border
router", implementing the bitcoindx wire protocol and relay, making
...
The ZeroMQ feature in BitcoinDX Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoindxd:
...
For instance:
    ...
    $ bitcoindxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bitcoindxd.tx.raw \
...
These options can also be provided in bitcoindx.conf.
...
From the perspective of bitcoindxd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoindxd directly. Furthermore, no information is
...
using. BitcoinDXd appends an up-counting sequence number to each
