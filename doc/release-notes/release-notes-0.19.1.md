...
BitcoinDX Core version 0.19.1 is now available from:

  <https://bitcoin-dx.org/bin/bitcoindx-core-0.19.1/>
...
From BitcoinDX Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as BitcoinDX Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, BitcoinDX Core does not yet change appearance when
...
- #17488 fix "bitcoindxd already running" warnings on macOS (fanquake)
