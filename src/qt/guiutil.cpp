...
#include <qt/bitcoindxaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinDX address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitcoinDXAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinDXAddressCheckValidator(parent));
}
...
bool parseBitcoinDXURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitcoindx: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoindx"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinDXUnits::parse(BitcoinDXUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoinDXURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoinDXURI(uriInstance, out);
}

QString formatBitcoinDXURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitcoindx:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinDXUnits::format(BitcoinDXUnits::BTC, info.amount, false, BitcoinDXUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinDXConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BITCOINDX_CONF_FILENAME));
    ...
    /* Open bitcoindx.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinDX.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinDX (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitcoinDX (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinDX*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcoindx.desktop";
    return GetAutostartDir() / strprintf("bitcoindx-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitcoindx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinDX\n";
        else
            optionFile << strprintf("Name=BitcoinDX (%s)\n", chain);
